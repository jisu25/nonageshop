...
<%@ include file="sub_menu.jsp" %>       
 ...
 <th> 주문 총액</th>     
     ...
     <tr>  
       <td> <fmt:formatDate value="${orderDetail.order.orderDate}" type="date"/></td>
       <td> ${orderDetail.order.no} </td>    
       <td> ${orderDetail.order.member.name} </td>
 ...
 <th>가격</th> <th>처리 상태</th>    
     ...
     <tr>
       ...
       <td> <fmt:formatNumber type="currency" value="${orderD.product.salePrice*orderD.quantity}" /> </td>
       <td>
         <c:choose>
         <c:when test='${orderD.resYn=="1"}'> 진행중 </c:when>
