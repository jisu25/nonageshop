...
function go_ab() // 판매가-원가=순매출을 replace해서 계산해 준다.
{
	...
	var a = theForm.salePrice.value.replace(/,/g, '');
	var b = theForm.price.value.replace(/,/g, '');
	var ab = parseInt(a) - parseInt(b);
	theForm.margin.value = ab;
}
...
function go_save() 
{
	...
	if (theForm.kind.value == '') {
		...
		alert('원가를 입력하세요.');
		theForm.price.focus();
	} else if (theForm.salePrice.value == '') {
		alert('판매가를 입력하세요.');
		...
		theForm.price1.value = removeComma(theForm.price);
		theForm.price2.value = removeComma(theForm.salePrice);
		theForm.price3.value = removeComma(theForm.margin);
...
function go_detail(tpage, pseq) {
	...
	theForm.action =  "adminProductDetail.do?tpage=" + tpage + "&no=" + no;
	theForm.submit();
}
...
function go_mod(tpage, pseq) {
	...
	theForm.action = "adminProductUpdateForm.do&tpage=" + tpage + "&no=" + no;
	theForm.submit();
}
...
function go_mod_save(tpage, pseq) {
	...
	if (theForm.kind.value == '') {
		...
		if (confirm('수정하시겠습니까?')) {
			...
			if (theForm.delYn.checked == true) {
				theForm.delYn.value = "y";
			}
			...
			theForm.salePrice.value = removeComma(theForm.salePrice);
...
function go_mod_mov(tpage, pseq) {
	var theForm = document.frm;
	theForm.action = 'adminProductDetail?tpage=' + tpage + "&no=" + no;
	theForm.submit();
}
